...
function psipval=psideri(x,y,delta_x)//derivative of psival wrt y(new)
    term1=(1-Q^2/(B^2*g*y^3))^(-1);
    term2=(2*n^2*Q^2)/(B^2*y^3);
    term3=(B*y/(B+2*y))^(-4/3);
    term4=(4*n^2*Q^2)/(3*B^2*y^2);
    ...
    term10=(n^2*Q^2)/(B^2*y^2);
    term11=(B*y/(B+2*y))^(4/3);
    psipval=(1-delta_x*(term1*(term2*term3+term4*term5*term6)-term7*term8*(term9-term10*term11))-1)/(-delta_x);//using eqn(135.1)
    endfunction
//*********************************************
...
for i=2:mnode
    K1=delta_x*(1-1*delta_x*psideri(xc(i-1)+1*delta_x,yv(i-1),delta_x))^(-1)*psi(xc(i-1)+1*delta_x,yv(i-1));//using semi-implicit eqn (138) and putting values from (135.1) and (136.1)
    yv(i)=yv(i-1)+1*K1;
end
//plot
plot(xc,yv,"-b")
...
set(gca(),"auto_clear","off")
plot([0 Lx],[yc yc],'-b')
xtitle("GVF implicit Backward Euler Method","x axis(m)","flow depth(m)")
